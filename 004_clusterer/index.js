Rich.init(
    ...
    // Rich.ajaxJsonGet("test.json")
    Rich.getJS("test2.js").then(data => {
        console.log(data)
        ...
        while (i--) {
            ...
            let tData = positions[i]
            let ttt = `
                ${tData.profile}(${tData.month}.${tData.day})<br>
                ${tData.full}
            `
            ...
            markers[i] = new kakao.maps.CustomOverlay({
                map: map, // 인포윈도우가 표시될 지도
                position: new kakao.maps.LatLng(...(tData).latlng.split(',')),
                content: content
            });
